...
for args in $all_args; do
    ...
    stream_in="pos_${file_name//[^0-9]/}_in_file.txt"

    if grep -q '^st ' "$args"; then
        out_file="pos_${file_name//[^0-9]/}_out_file.txt"
    ...
    if "$path_dir"/pos_case.sh "$stream_in" "$out_file" "$args"; then
    ...
    rm -f "$data_dir"/pos_*_in_file_n.txt
done
...
for args in $all_args; do
    ...
    file="neg_${file_name//[^0-9]/}_in_file.txt"

    if "$path_dir"/neg_case.sh "$file" "$args"; then
    ...
    rm -f "$data_dir"/neg_*_in_file_n.txt
done
